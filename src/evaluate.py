...
def _parse_seed_list(value: str | None, default_seed: int) -> list[int]:
    if value is None:
        return [default_seed]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", choices=["synthetic", "cifar10c"], default=None)
    parser.add_argument("--cifar10c-dir", default=None)
    ...
    parser.add_argument("--full-test", action="store_true", help="Evaluate all 10,000 CIFAR-10 test images.")
    parser.add_argument("--tta-lr", type=float, default=None)
    parser.add_argument("--tta-steps", type=int, default=None)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--seeds", default=None, help="Comma-separated evaluation seeds, e.g. 0,1,2.")
...
def main() -> None:
    ...
    default_seed = args.seed if args.seed is not None else cfg["seed"]
    seeds = _parse_seed_list(args.seeds, default_seed)
    ...
    eval_cfg = cfg["eval"]
    tta_cfg = cfg["tta"]
    dataset_name = args.dataset or eval_cfg.get("dataset", "synthetic")
    cifar10c_dir = args.cifar10c_dir or eval_cfg.get("cifar10c_dir")
    ...
    elif dataset_name == "cifar10c" and cifar10c_dir is None:
        raise ValueError("--cifar10c-dir is required for CIFAR-10-C evaluation.")
    ...
    for seed in seeds:
        ...
        for corruption, severity in tqdm(eval_jobs, desc=f"evaluation jobs seed={seed}"):
                    ...
                    rows.append(
                        {
                            ...
                            "runtime_per_batch_sec": result.runtime_per_batch_sec,
